 ...
 */
public class IntoCartTask extends AsyncTask<String, Void, String> {

    private Context activity;
    private ProgressBar progressBar;

    public IntoCartTask(Context theActivity, ProgressBar theProgressBar) {
    ...
    protected String doInBackground(String... args) {
        String url = activity.getString(R.string.fetch_into_cart_head);
        List<NameValuePair> params = new ArrayList<NameValuePair>();
        params.add(new BasicNameValuePair("userId", args[0]));
        params.add(new BasicNameValuePair("itemId", args[1]));
        params.add(new BasicNameValuePair("number", args[2]));
